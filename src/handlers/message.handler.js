...
export class MessageHandler {
  ...
  static ALLOWED_CONTACTS = ["256783604580", "256785866559", "256758662135", "256787250196", "256706943977", "256778687196", "256752493532", "256707675112", "256759552604", "256705054692", "256702547447", "256777676206", "256750785703", "256701259245", "256726078658", "256751531151", "256757104399", "256781533158", "256702769822"];
  ...
  static async handleIncomingWithQueue(payload) {
    try {
      ...
      switch (message.type) {
        case 'text':
          await this.processTextMessage(message, contact, businessPhoneNumberId, session);
          break;
        case 'interactive':
          if (message.interactive.type === 'list_reply') {
            await this.processListReply(message, contact, businessPhoneNumberId, session);
          }
          if (message.interactive.type === 'nfm_reply') {
            await this.processFormReply(message, contact, businessPhoneNumberId, session);
          }
          break;
        case 'location':
          await this.processLocationResponse(message, contact, businessPhoneNumberId, session);
          break;
        default:
  ...
  static async processTextMessage(message, contact, businessPhoneNumberId, session) {
    try {
      const userPhone = contact.wa_id;
      const userName = contact.profile.name;
      ...
      if (!this.ALLOWED_CONTACTS.includes(userPhone)) {
        await this.sendDevelopmentMessage(businessPhoneNumberId, userName, userPhone);
        return;
      }
  ...
  static async processListReply(message, contact, businessPhoneNumberId, session) {
    try {
      // Mark message as read
      await WhatsAppService.markMessageAsRead(businessPhoneNumberId, message.id);
      ...
      switch (list_Id) {
        case "pay_tax":
        case "pay_utilities":
          await this.sendBillsPaymentFlowMessage("442394835264933", message, businessPhoneNumberId);
          break;
        case "open_account":
          await this.sendOpenAccountFlowMessage("1819890832152763", message, businessPhoneNumberId);
          break;
        case "pay_merchant":
          await this.sendMerchantPaymentFlowMessage("393839823814268", message, businessPhoneNumberId);
          break;
        case "report_complaint":
        case "send_inquiry":
          await this.sendCustomerSupportFlowMessage("2102552853524929", message, businessPhoneNumberId);
          break;
        default:
  ...
  static async processLocationResponse(message, contact, businessPhoneNumberId, session) {
    try {
      ...
      const result = await database.insertOrUpdateUserLocation(userPhone, latitude.toString(), longitude.toString(), fileUrl);
      ...
      await WhatsAppService.sendMessage(businessPhoneNumberId, userPhone, user_message, message.id);
  ...
  static async processFormReply(message, contact, businessPhoneNumberId, session) {
    try {
      // Mark message as read
      await WhatsAppService.markMessageAsRead(businessPhoneNumberId, message.id);
  ...
  static async sendBillsPaymentFlowMessage(flow_id, message, businessPhoneNumberId) {
    const receiver_number = message.from;
    await FlowService.sendBillsPaymentFlow(flow_id, receiver_number, businessPhoneNumberId)
  }

  static async sendOpenAccountFlowMessage(flow_id, message, businessPhoneNumberId) {
    const receiver_number = message.from;
    await FlowService.sendAccountOpenningFlow(flow_id, receiver_number, businessPhoneNumberId)
  }

  static async sendMerchantPaymentFlowMessage(flow_id, message, businessPhoneNumberId) {
    const receiver_number = message.from;
    await FlowService.sendMerchantPaymentFlow(flow_id, receiver_number, businessPhoneNumberId)
  }

  static async sendCustomerSupportFlowMessage(flow_id, message, businessPhoneNumberId) {
    const receiver_number = message.from;
    await FlowService.sendCustomerSupportFlow(flow_id, receiver_number, businessPhoneNumberId)
  }


  // Send development stage message
  static async sendDevelopmentMessage(businessPhoneNumberId, userName, userPhone) {
    await WhatsAppService.sendMessage(
      businessPhoneNumberId,
      userPhone,
  ...
  static async showServices(message, businessPhoneNumberId) {
    try {
      const message_body = "Effortless banking is just a tap away. Choose from a range of convenient services at your fingertips..";
      const data = await WhatsAppService.sendInteractiveListReplyMessage(message_body, message.from, businessPhoneNumberId);
      console.log(data)
    } catch (error) {
      await WhatsAppService.sendMessage(
        businessPhoneNumberId,
